...
import AddVoltage from "./AddVoltage";
import EditVoltage from "./EditVoltage";
...
export default function VoltageTable() {
  ...
  const [data, setData] = useState<Voltage[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/voltages`);
      setData(response.data);
    } catch (err) {
      console.error("Error loading Voltages:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axiosInstance.delete(`/voltages/${deleteId}`);
      await fetchData();
                ...
                {table.getRowModel().rows.map((row) => (
                  <tr key={row.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700">
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="px-4 py-3 dark:text-gray-200 whitespace-nowrap">
                        {flexRender(
