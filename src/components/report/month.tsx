...
export default function MonthTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      url = selectedPowerId
        ? `/reports/month?powerId=${selectedPowerId}&sYear=${selectedYear}&sMonth=${selectedMonth}`
        : `/reports/month?sYear=${selectedYear}&sMonth=${selectedMonth}`;
  ...
  const exportToPDF = () => {
    ...
    const url = `/api/report/pdf/month?${params}`;
  ...
  const columns: ColumnDef<Month>[] = [
    ...
    //             onClick={() =>
    //               router.push(
    //                 `/${user?.roleId === 3 || user?.roleId === 4 ? "dispatch" : "declaration"}/month/monthview/${encryptId(row.original.id)}`,
    //               )
    //             }
    ...
    //             onClick={() =>
    //               router.push(
    //                 `/${user.roleId === 4 ? "dispatch" : "declaration"}/month/monthaction/${encryptId(row.original.id)}`,
    //               )
    //             }
