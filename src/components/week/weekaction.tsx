...
export default function WeekAction() {
  const { id } = useParams();
  const [data, setData] = useState<WeekPowerData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosInstance.get(`/weekpowers/${decryptedId}`);
  ...
  const handleAcknowledge = async () => {
    try {
      setLoading(true);
      await axiosInstance.put(`/weekpowers/acknowleged/${data?.id}`);
      ...
      <div className="mb-4 flex items-center justify-between gap-4">
        {(user?.roleId === 4 || user?.roleId === 6) && (
          <button
            onClick={() =>
              router.push(
                `/${user.roleId === 4 ? "dispatch" : "declaration"}/week`,
              )
            }
        ...
        {(user?.roleId === 4 || user?.roleId === 6) && (
          <button
            onClick={() => {
              if (data?.id != null) {
                router.push(
                  `/${user.roleId === 4 ? "dispatch" : "declaration"}/week/revise/${encryptId(data.id)}`,
                );
              }
            }}
