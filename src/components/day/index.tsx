...
export default function DayTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      url = selectedPowerId
        ? `/daypowers?powerId=${selectedPowerId}&startDate=${start}&endDate=${end}`
        : `/daypowers?startDate=${start}&endDate=${end}`;
          ...
          {(user?.roleId === 3 ||
            ...
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user?.roleId === 3 || user?.roleId === 4 ? "dispatch" : "declaration"}/day/dayview/${encryptId(row.original.id)}`,
                  )
                }
          ...
          {(user?.roleId === 4 || user?.roleId === 6) && (
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user.roleId === 4 ? "dispatch" : "declaration"}/day/dayaction/${encryptId(row.original.id)}`,
                  )
                }
  ...
  const CreatePage = () => {
    router.push("/declaration/day/create");
  };
