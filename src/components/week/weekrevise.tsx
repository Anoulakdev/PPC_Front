...
export default function WeekRevise() {
  const { id } = useParams();
  ...
  const handleWeeklyRemarkChange = (value: string) => {
    setData({
      ...data,
      powerCurrent: {
        ...data.powerCurrent,
  ...
  const handleRevise = async () => {
    try {
      ...
      await axiosInstance.put(`/weekpowers/revise/${data?.id}`, payload);
  ...
  return (
    <div className="rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900 dark:text-gray-100">
      <h1 className="mb-5 text-center text-xl font-bold">
        Weekly Availability Declaration (Revise)
      </h1>
        ...
        <>
          ...
          <div className="mt-4">
            <textarea
              ...
              onChange={(e) => handleWeeklyRemarkChange(e.target.value)}
