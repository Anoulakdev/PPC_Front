...
import AddEvent from "./AddMaintenance";
import EditEvent from "./EditMaintenance";
...
export default function UserTable() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get("/maintenances/dam");
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axiosInstance.delete(`/maintenances/${deleteId}`);
      await fetchData();
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <button
            onClick={() =>
              router.push(`/maintenance/${encryptId(row.original.id)}`)
            }
  ...
  return (
    <>
      <div className="rounded-xl bg-white p-4 shadow-lg transition-colors dark:bg-gray-800 dark:text-gray-100">
        <div className="mb-3 flex items-center justify-between">
          {user?.roleId === 6 ? (
            <button
              onClick={openAddModal}
              className="rounded-md border-blue-700 bg-blue-500 px-3 py-2 text-sm text-white hover:bg-blue-600 dark:bg-blue-600 dark:text-white"
            >
              Add Maintenance
            </button>
          ) : (
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto rounded-lg border">
            <table className="min-w-full rounded-lg border text-left">
              <thead className="text-md bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-100">
                  ...
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th
                        ...
                        className="cursor-pointer px-3 py-3 text-left whitespace-nowrap select-none"
                      >
                        {flexRender(
