...
export const CardDayAll = () => {
  const [data, setData] = useState<TotalPowerData | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axiosInstance.get(`/daypowers/totalpowerall`);
        setData(res.data);
      } catch (error) {
        ...
        <div className="mt-6 flex w-full items-center justify-between gap-4">
          <div className="text-left">
            ...
            <p className="mt-5 text-lg font-semibold text-gray-800 dark:text-white/60">
              {formatNumber(data.yesterday?.totalOriginal)} MWh
            </p>
          </div>
          ...
          <div className="text-right">
            ...
            <p className="mt-5 text-lg font-semibold text-gray-800 dark:text-white/60">
              {formatNumber(data.yesterday?.totalCurrent)} MWh
            </p>
          </div>
        </div>
      </div>
