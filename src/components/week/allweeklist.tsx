...
export default function WeekTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      url = selectedPowerId
        ? `/weekpowers/alldocument?powerId=${selectedPowerId}&sYear=${selectedYear}&sWeek=${selectedWeek}`
        : `/weekpowers/alldocument?sYear=${selectedYear}&sWeek=${selectedWeek}`;
          ...
          {(user?.roleId === 3 ||
            ...
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user?.roleId === 3 || user?.roleId === 4 ? "alldocument/edl" : "alldocument/power"}/week/weekview/${encryptId(row.original.id)}`,
                  )
                }
