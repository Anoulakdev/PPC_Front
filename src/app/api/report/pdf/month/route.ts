...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sMonth = searchParams.get("sMonth");
    ...
    const apiUrl = powerId
      ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/reports/month?powerId=${powerId}&sYear=${sYear}&sMonth=${sMonth}`
      : `${process.env.NEXT_PUBLIC_API_BASE_URL}/reports/month?sYear=${sYear}&sMonth=${sMonth}`;
...
const getMonthDaysWithWeekday = (year: number, month: number) => {
  const date = new Date(year, month - 1, 1); // ✅ แปลงเป็น 0-based
  const days: string[] = [];
  ...
  while (date.getMonth() === month - 1) {
    const weekday = formatter.format(date).toUpperCase(); // MON, TUE
...
function generatePage(item: any) {
  ...
  const monthDays = getMonthDaysWithWeekday(
    Number(item.sYear),
    Number(item.sMonth),
  );
  ...
  return `
    <div class="page">
      ...
      <div class="header">
        ...
        <h2>Monthly Availability and Declaration</h2>
      </div>
      ...
      <div class="info-row">
        ...
        <div class="info-item" style="margin: 0 5px;">
          <strong>Declaration for Month:</strong> ${item.sMonth || "-"} / ${item.sYear || "-"}
        </div>
        <div class="info-item">
...
function generateHourlyRows(turbines: any[], remarks: string[] = []) {
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    rows.push(`
      <tr>
        <td style="font-size: 5pt;">${timeRange}</td>
