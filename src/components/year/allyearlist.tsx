...
export default function YearTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      url = selectedPowerId
        ? `/yearpowers/alldocument?powerId=${selectedPowerId}&sYear=${selectedYear}`
        : `/yearpowers/alldocument?sYear=${selectedYear}`;
          ...
          {(user?.roleId === 3 ||
            ...
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user?.roleId === 3 || user?.roleId === 4 ? "alldocument/edl" : "alldocument/power"}/year/yearview/${encryptId(row.original.id)}`,
                  )
                }
