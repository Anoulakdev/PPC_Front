...
export default function YearTable() {
  ...
  const { selectedPowerId, selectedYear, setSelectedPowerId, setSelectedYear } =
    useFilterStore();
  ...
  const fetchData = async () => {
    try {
      ...
      url = selectedPowerId
        ? `/yearpowers?powerId=${selectedPowerId}&sYear=${selectedYear}`
        : `/yearpowers?sYear=${selectedYear}`;
          ...
          {(user?.roleId === 3 ||
            ...
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user?.roleId === 3 || user?.roleId === 4 ? "dispatch" : "declaration"}/year/yearview/${encryptId(row.original.id)}`,
                  )
                }
          ...
          {(user?.roleId === 4 || user?.roleId === 6) && (
            <div className="group relative inline-block">
              <button
                onClick={() =>
                  router.push(
                    `/${user.roleId === 4 ? "dispatch" : "declaration"}/year/yearaction/${encryptId(row.original.id)}`,
                  )
                }
  ...
  const CreatePage = () => {
    router.push("/declaration/year/create");
  };
